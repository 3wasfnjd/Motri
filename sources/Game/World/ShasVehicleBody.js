...
export function buildShasVehicleBody(paintMaterial, detailMaterial)
{
    ...
    const palette = { trim: '#27312e', glass: '#20373d', metal: '#a8b1a6',
    ...
    box([.96,.237,0], [.67,.024,.85], 'paint', undefined, .006)
    ...
    for(const sign of [-1,1])
    {
        ...
        rod([.289,.244,sign*.632],[.20,.472,sign*.632],.009,'detail',palette.trim)
        box([-.297,.111,sign*.681],[.128,.032,.025],'detail',palette.metal,.005)
    ...
    {
        for(const [i,ch] of [...'TOYOTA'].entries()) for(const stroke of glyphs[ch])
    ...
    group.name = 'Shas_BodyStyle'
    group.userData = { bodyOnly: true, forward: '+X', paintedColor: '#c9b58d' }
    for(const [role, geometries] of Object.entries(pieces))
    {
        ...
        mesh.name = role === 'paint' ? 'Shas_BodyPaint' : 'Shas_BodyDetails'
