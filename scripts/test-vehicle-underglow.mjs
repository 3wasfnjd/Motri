...
// Loading and the entire reveal transition must keep both effects off.
assert.equal(glow.strips.visible, false)
glow.update(vehicle)
assert.equal(glow.strips.visible, false)
assert.equal(vehicleUnderglow.strength.value, 0)
glow.update(vehicle, false)
assert.equal(glow.strips.visible, false)
assert.equal(vehicleUnderglow.strength.value, 0)
const snapshot = JSON.stringify(vehicle)
glow.update(vehicle, true)
...
assert.equal(glow.strips.visible,true)
glow.update(vehicle,false)
assert.equal(glow.strips.visible,false)
assert.equal(vehicleUnderglow.strength.value,0)
glow.update(vehicle,true)
